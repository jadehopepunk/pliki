  ...
  module Helpers
    class FormBuilder
      ...
      def textile_editor(method, options = {})
        @template.textile_editor(@object_name, method)
      end
...
class PlikiFormBuilder < ActionView::Helpers::FormBuilder
  ...
  (field_helpers + [:select, :textile_editor]).uniq.each do |name|
    create_labelled_field(name) unless name == 'hidden_field'
  end
    
end

ActionView::Base.default_form_builder = PlikiFormBuilder
